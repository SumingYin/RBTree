...
//            // 获取删除节点的父节点、儿子节点信息
//            Node dNParent = deleteNode.parent;
//            int dNPosition = deleteNode.position;
//            Node leftDsub = null;
//            Node rightDsub = null;
//            if (deleteNode.leftSub != null) {
//                leftDsub = deleteNode.leftSub;
//            }
//            if (deleteNode.rightSub != null) {
//                rightDsub = deleteNode.rightSub;
//            }
//
//            // 寻找替换的节点，将删除的节点进行值替换即可
//            if (leftDsub == null && rightDsub == null) {
//                // 如果该节点为红色,没有可替换的节点，直接删除
//                if (deleteNode.color == 0) {
//                    nomalDelete(deleteNode);
//                    return true;
//                } else {
...
//                        // 下面进行分类讨论
//                        // 1、兄弟是黑色
//                        Node dSub = deleteNode.getAnotherSub(deleteNode);
//                        // 可以删除该节点
//                        if(!isDeleted){
//                            nomalDelete(deleteNode);
//                            isDeleted = true;
...
//                        if (dSub.color == 1) {
//                            // 1.1 父节点是红色，兄弟无子节点
//                            if (dNParent.color == 0 && dSub.leftSub == null && dSub.rightSub == null) {
//                                dNParent.color = 1;
//                                dSub.color = 0;
//
//                            } else if (dNParent.color == 1 && dSub.leftSub == null && dSub.rightSub == null) {
//                                // 父节点是黑色，兄弟无子节点，该种情况需要递归处理
//                                // 需要对parent进行调整
//                                dSub.color = 0;
//                            } else if (dSub.leftSub != null && dSub.leftSub.color == 0) {
//                                // 右旋
//                                nomalRightRotate(dSub);
//                                // 变换颜色
//                                Node originalLeftSub = dSub.parent;
//                                originalLeftSub.color = dNParent.color;
//                                dNParent.color = 1;
//                                // 根据位置进行左旋或右旋
//                                if (originalLeftSub.position == 1) {
//                                    nomalRightRotate(dNParent);
//                                } else if (originalLeftSub.position == 2) {
//                                    nomalLeftRotate(dNParent);
//                                }
//
//                            } else if (dSub.rightSub != null && dSub.rightSub.color == 0) {
//                                // 兄弟右孩子是红色
//                                dSub.color = dNParent.color;
//                                dNParent.color = 1;
//                                if (dSub.position == 1) {
//                                    nomalRightRotate(dNParent);
//                                } else if (dSub.position == 2) {
//                                    nomalLeftRotate(dNParent);
//                                }
//                                dSub.rightSub.color = 1;
//
//                            }
//                        } else if (dSub.color == 0) {
//                            // 此时parent必为黑，两个侄子肯定不为null
//                            if (dSub.position == 1) {
//                                nomalRightRotate(dNParent);
//                            } else if (dSub.position == 2) {
//                                nomalLeftRotate(dNParent);
...
//                }
//
//            } else if ((rightDsub == null && leftDsub != null) || (rightDsub != null && leftDsub == null)) {
//                // deleteNode 存在一个孩子
//                // 此时deleteNode 节点必然是黑色的
//                Node tempNode = nomalDelete(deleteNode);
//                tempNode.color = 1;
//            } else if (rightDsub != null) {
//                // 存在两个孩子，rightDsub 必不为null
//                // 找到替换的节点
//                Node tempRight = rightDsub;
//                Node tempChange = tempRight;
//                Node x = null;
//
//                while (tempChange.leftSub != null) {
...
//                    // 存在右孩子
//                    x = tempChange.rightSub;
//                    if (x != null) {
//                        // tempChange.color必然为1，黑色
//                        x.color = 1;
//                        // 此时可删除
//                        if(!isDeleted){
//                            nomalDelete(deleteNode);
...
//                } else {
//                    // 此时不相等，有可能为单孩子，或没有孩子
//                    Node tempChangeP = tempChange.parent;
//                    if (tempChange.rightSub != null) {
//                        // 单孩子，要删除的节点必然是黑色
//                        Node temp = nomalDelete(deleteNode);
//                        temp.color = 1;
//                    } else {
//                        // 不存在右子树
//                        Node dSub = deleteNode.getAnotherSub(deleteNode);
